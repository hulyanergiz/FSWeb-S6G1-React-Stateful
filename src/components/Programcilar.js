...
export default function Programcilar() {
  const [programcilar, setProgramcilarListesi] = useState(enIyilerListesi);
  const [oneCikaninId, setOneCikaninId] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    if (oneCikaninId) {
      return programcilar.find((dev) => dev.id === oneCikaninId).isim;
    } else {
      return "";
    }
  };
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: oneCikaninId ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
